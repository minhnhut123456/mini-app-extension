...
try {
  ...
  // ---- VSCE PACKAGE ----
  console.log("▶ Running VSCE package from ROOT but inside DEST...");

  execSync("pnpm vsce package", {
    cwd: DEST,
    stdio: "inherit",
  });

  // ---- MOVE VSIX ----
  console.log("▶ Moving generated VSIX to ROOT...");

  const files = fs.readdirSync(DEST).filter((f) => f.endsWith(".vsix"));
  if (files.length === 0) {
    throw new Error("❌ VSIX file not found in isolated folder!");
  }

  const vsixFile = files[0];
  const srcVsix = path.join(DEST, vsixFile);
  const destVsix = path.join(ROOT, vsixFile);

  // Move
  fs.renameSync(srcVsix, destVsix);
  console.log(`✓ Moved VSIX: ${destVsix}`);
  console.log("✓ Done. VSIX ready in root folder.");
} catch (error) {
